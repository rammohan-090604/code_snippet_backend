    ...
    try {
        ...
        if (user) {
            const updateResult = await collection.updateOne(
                { email },
                {
                    $push: { codes: { name, language, code, date: new Date() } }
                }
            );
            console.log(`${updateResult.modifiedCount} document(s) updated with new code.`);
            return { message: "Code added to existing user." };
...
app.get('/api/getFullCode', async (req, res) => {
    ...
    res.setHeader('Content-Type', 'text/plain'); // Assuming plain text for now
    res.send(result.code); // Send the code content
});
